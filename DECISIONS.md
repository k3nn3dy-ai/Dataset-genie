...
| 10 | `make` uses `uv run --no-sync`, and `make install` writes a plain `genie_dev.pth` and unhides uv's `.pth` files. | Homebrew Python on macOS skips `.pth` files carrying the hidden flag, which uv sets when it rewrites the editable install; this made `import genie` fail intermittently while several teammates ran `uv run` concurrently. |
