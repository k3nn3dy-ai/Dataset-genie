...
export function EstimateModal({ open, onClose, onConfirm, estimate, loading, error, spend, cap, stageTitle, running, runError, onForce, onSettings, onOpenStage }: Props) {
  ...
  return (
    ...
    >
      {runError && (
        <Banner tone={runError.kind === 'nothing' ? 'green' : runError.kind === 'overcap' || runError.kind === 'conflict' ? 'amber' : 'red'} className="mb-4">
          {runError.kind === 'nokey' ? <>No OpenRouter API key is set. {onSettings && <button type="button" className="underline text-green" onClick={onSettings}>Set your key in Settings</button>} and try again.</>
            : runError.kind === 'conflict' ? <>A run is already in progress for this project{runError.stage ? ` (stage ${String(runError.stage).padStart(2, '0')})` : ''} — {runError.stage && onOpenStage ? <button type="button" className="underline text-green" onClick={() => onOpenStage(runError.stage!, runError.runId)} data-testid="open-running-stage">open it</button> : 'wait for it to finish'}.</>
      ...
      {estimate && !loading && (
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-3 gap-2">
            <StatTile label="estimated cost" value={usd(estimate.est_usd)} tone={over ? 'amber' : 'green'} />
          ...
          {over
            ...
            : <Banner tone="green" icon="lock">Cap is enforced server-side; the run auto-stops at {Math.round(90)}% of the cap. Actual usage is billed from provider usage, not this estimate.</Banner>}
        </div>
      )}
    </Modal>
  )
