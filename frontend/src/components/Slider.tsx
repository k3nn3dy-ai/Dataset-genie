...
interface Props {
  ...
  className?: string
  disabled?: boolean
  tone?: 'orange' | 'steel' | 'amber'
}

export function Slider({ label, value, onChange, min = 0, max = 100, step = 1, format, className, disabled, tone = 'orange' }: Props) {
  const fill = ((value - min) / (max - min)) * 100
  const colour = tone === 'orange' ? '#ff6a1a' : tone === 'steel' ? '#b3b3b3' : '#ffa040'
    ...
    <div className={clsx('flex flex-col gap-1.5', disabled && 'opacity-40', className)}>
      {(label || format) && (
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="font-mono text-[12px] text-orange tabular-nums">{format ? format(value) : value}</span>
        </div>
      )}
      <input
