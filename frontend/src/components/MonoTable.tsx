            ...
            {columns.map((c) => {
              ...
              return (
                <th
                  ...
                  className={clsx(cell, 'label !text-[10px] text-left font-normal whitespace-nowrap select-none', c.align === 'right' && 'text-right', c.align === 'center' && 'text-center', c.sortValue && 'cursor-pointer hover:text-text', active && '!text-cyan')}
            ...
            return (
              <tr
                ...
                className={clsx(
                  ...
                  onRowClick && 'cursor-pointer hover:bg-cyan/5',
                  activeKey === k && 'bg-cyan/10 shadow-[inset_2px_0_0_#00f0ff]',
                  isSel && 'bg-cyan/10',
...
export function Check({ checked, onChange, className }: { checked: boolean; onChange: () => void; className?: string }) {
  return (
    <button
      ...
      className={clsx('w-3.5 h-3.5 rounded-[3px] border flex items-center justify-center transition-colors focus-ring', checked ? 'bg-cyan border-cyan text-bg' : 'border-line2 bg-bg/60 hover:border-cyan/60', className)}
    >
