      ...
      {snap ? (
        <div className="p-4 flex flex-col gap-3 border-b border-line">
          <div className="grid grid-cols-4 gap-2">
            <StatTile size="sm" label="done / total" value={`${snap.done}`} unit={`/ ${snap.total}`} tone="green" />
            ...
            {workers.map(([id, w]) => (
              <span
                ...
                className={clsx('h-4 min-w-[26px] px-1 rounded-chip border font-mono text-[9px] flex items-center justify-center',
                  ...
                  w.status === 'done' && 'border-green/50 text-green',
            ...
            <div className="rounded-btn border border-line bg-bg/60 max-h-[220px] overflow-y-auto p-2 font-mono text-[11px] leading-[1.5]">
              ...
              {(snap?.log ?? []).map((l, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <span className={clsx('shrink-0 w-10 uppercase', l.level === 'error' ? 'text-red' : l.level === 'warn' ? 'text-amber' : l.level === 'debug' ? 'text-dim' : 'text-green')}>{l.level}</span>
                  ...
                  <tbody>
                    {log.data.items.slice(0, 60).map((c) => (
                      <tr key={c.id} className="border-t border-line/60">
                        ...
                        <td className="px-2 py-[3px] text-right tabular-nums">{c.tokens_in}/{c.tokens_out}</td><td className="px-2 py-[3px] text-right tabular-nums">{c.latency_ms}</td><td className="px-2 py-[3px] text-right tabular-nums text-green">{c.cost_usd.toFixed(4)}</td>
