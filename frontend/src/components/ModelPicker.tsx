    ...
    <div ref={ref} className={clsx('flex flex-col gap-1.5 min-w-0', className)}>
      ...
      <div className="relative">
        <button
          ...
          className={clsx('field w-full h-10 flex items-center gap-2.5 text-left', open && '!border-green/60', warn && '!border-amber/60')}
        >
          <span className="w-6 h-6 rounded-[5px] bg-surface2 border border-line2 flex items-center justify-center text-green shrink-0"><Icon name="sparkle" size={12} /></span>
            ...
            <div className="max-h-[260px] overflow-y-auto">
              ...
              {list.map((m) => (
                <button
                  ...
                  className={clsx('w-full grid grid-cols-[1fr_auto_auto_auto] gap-x-3 items-center px-3 py-1.5 text-left hover:bg-green/10 border-t border-line/50', m.id === value.slug && 'bg-green/10')}
                >
                  ...
                  <span className="font-mono text-[11px] text-green tabular-nums text-right">{m.prompt_price_per_m.toFixed(2)}</span>
