  ...
  return (
    <div className="rounded-card border border-line bg-bg/40 flex flex-col">
      <div className="px-3 py-2 border-b border-line flex items-start gap-3">
        ...
        <div className="flex gap-1 shrink-0"><Chip tone="magenta">{pair.metadata.flaw}</Chip><Chip tone="dim">{pair.metadata.strategy}</Chip></div>
      </div>
      <div className="grid grid-cols-2 divide-x divide-line">
        <Side title="chosen" tone="acid" ops={diff.left} />
      ...
      {pair.metadata.judge?.rationale && (
        <div className="px-3 py-2 border-t border-line flex gap-3 items-start">
          ...
          <Chip tone={pair.metadata.judge.verdict === 'tie' ? 'magenta' : 'acid'}>{pair.metadata.judge.verdict}</Chip>
        </div>
      )}
    </div>
  )
...
function Side({ title, tone, ops }: { title: string; tone: 'acid' | 'red'; ops: DiffOp[] }) {
  return (
    <div className="p-3 min-w-0">
      <div className={`label mb-2 ${tone === 'acid' ? '!text-acid' : '!text-red'}`}>{title}</div>
      <div className="text-[13px] leading-relaxed whitespace-pre-wrap max-h-[300px] overflow-y-auto font-ui">
        {ops.map((op, i) => op.kind === 'same'
          ...
          : <mark key={i} className={op.kind === 'del' ? 'bg-acid/20 text-acid rounded-[2px] px-[1px]' : 'bg-red/25 text-red rounded-[2px] px-[1px] line-through decoration-red/60'}>{op.text}</mark>)}
      </div>
    </div>
  )
}
