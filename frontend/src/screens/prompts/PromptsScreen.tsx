...
export function PromptsScreen() {
  ...
  const config = draft ? (
    <>
      <Panel title="Prompt model"><ModelPicker value={draft.model} onChange={(m) => setDraft({ model: m })} /></Panel>
      ...
      <Panel title="Sampling">
        <div className="flex flex-col gap-4">
          ...
          <Slider label="adversarial share" value={draft.adversarial_pct} min={0} max={50} step={0.5} format={(v) => `${v}%`} tone="steel" onChange={(v) => setDraft({ adversarial_pct: v })} />
  ...
  const results = prompts.isLoading ? <Spinner /> : items.length === 0 ? (
    <EmptyState title="No prompts yet" body="Run stage 02 to sample user prompts for every leaf using the personas and style mix on the left." />
  ) : (
              ...
              {ps.map((p) => (
                <li key={p.id} className="px-3 py-2 flex gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <Chip tone="dim">{p.style}</Chip><Chip tone="default">{p.persona}</Chip>{p.adversarial && <Chip tone="steel">adversarial</Chip>}
                  </div>
                </li>
              ))}
            </ul>
