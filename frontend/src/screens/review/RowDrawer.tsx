...
export function RowDrawer({ projectId, rowId, onClose, onNav }: Props) {
  ...
  return (
    <aside className="fixed top-0 right-0 bottom-0 z-[70] w-[620px] max-w-[60vw] panel !rounded-none border-l border-cyan/30 shadow-[-20px_0_60px_rgba(0,0,0,.6)] flex flex-col drawer-in">
      <header className="flex items-center gap-2 px-4 h-12 border-b border-line shrink-0">
        ...
        <span className="font-mono text-[12px] text-cyan truncate flex-1">{rowId}</span>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            ...
            {r.metadata.judge && (
              <section className="rounded-card border border-line bg-bg/40 p-3 flex flex-col gap-2">
                <div className="flex items-center justify-between"><span className="label">judge · {r.metadata.models.judge}</span><span className={`font-mono text-[20px] tabular-nums ${r.metadata.judge.score < 3 ? 'text-amber' : 'text-acid'}`}>{r.metadata.judge.score.toFixed(1)}<span className="text-dim text-[11px]"> / 5</span></span></div>
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(r.metadata.judge.criteria).map(([k, v]) => (
                    <div key={k} className="rounded-btn border border-line px-2 py-1.5"><div className="label !text-[9px] truncate">{k}</div><div className="flex gap-0.5 mt-1">{[1, 2, 3, 4, 5].map((i) => <span key={i} className={`h-1.5 flex-1 rounded-full ${i <= v ? 'bg-cyan' : 'bg-line2'}`} />)}</div></div>
                  ))}
                </div>
