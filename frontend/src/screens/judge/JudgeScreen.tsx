...
export function JudgeScreen() {
  ...
  const results = summary.isLoading ? <Spinner /> : !s || s.judged === 0 ? (
    <EmptyState title="No scores yet" body="Run stage 05 to score every response 1–5 per criterion with a one-line rationale. Pairs are judged blind in random A/B order." />
  ) : (
    <div className="flex flex-col gap-3">
      ...
      <Panel title="Score distribution · 10 bins"><Histogram bins={s.histogram} threshold={draft?.low_score_threshold} mean={s.mean} /></Panel>
      <MonoTable rows={items} rowKey={(r) => r.metadata.id} maxHeight="300px" defaultSort={{ key: 'score', dir: 'asc' }}
        ...
        columns={[
          ...
          { key: 'score', header: 'score', align: 'right', render: (r) => <span className={(r.metadata.judge?.score ?? 0) < (draft?.low_score_threshold ?? 3) ? 'text-amber' : 'text-ok'}>{r.metadata.judge?.score.toFixed(1)}</span>, sortValue: (r) => r.metadata.judge?.score ?? 0 },
