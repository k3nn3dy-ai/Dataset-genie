...
/** Mini heatmap: rows = teacher models, columns = topics, cell colour = refusal rate (cyan → amber → red). */
export function RefusalHeatmap({ cells }: { cells: RefusalCell[] }) {
  ...
  const colour = (rate: number) => rate === 0 ? 'rgba(0,240,255,.10)' : rate < 0.1 ? `rgba(0,240,255,${0.18 + rate * 3})` : rate < 0.4 ? `rgba(255,176,32,${0.25 + rate})` : `rgba(255,59,92,${0.35 + rate * 0.5})`
              ...
              {topics.map((t) => {
                ...
                return (
                  <td key={t} className="h-8 min-w-[72px] rounded-chip text-center tabular-nums" style={{ background: colour(rate) }} title={c ? `${c.refusals}/${c.total} refused` : 'no rows'}>
                    {c ? <span className={rate >= 0.4 ? 'text-text' : rate > 0 ? 'text-text/90' : 'text-cyan/70'}>{(rate * 100).toFixed(0)}%</span> : <span className="text-dim">—</span>}
                  </td>
                )
              })}
            </tr>
