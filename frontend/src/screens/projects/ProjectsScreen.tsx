...
export function ProjectsScreen() {
  ...
  return (
    <>
      <Header
        kicker="STAGE 00 · CHOOSE YOUR DOMAIN" kana={KANA.projects} title="Projects" numeral="00"
      ...
      {projects.data && list.length === 0 && (
        <EmptyState icon="folder" title="No projects yet" kana="空" body="Create a project from a preset. You can edit every stage's configuration before running it." action={{ label: 'New project', icon: 'plus', onClick: () => setOpen(true) }} />
      )}
      {list.length > 0 && (
        <>
          <div className="grid grid-cols-4 gap-3 mb-5">
            <StatTile label="projects" value={list.length} tone="cyan" />
            <StatTile label="total spend" value={usd(totalSpend)} tone="magenta" />
