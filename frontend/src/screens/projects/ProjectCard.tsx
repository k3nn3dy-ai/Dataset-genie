...
export function ProjectCard({ project, onDelete }: { project: Project; onDelete: () => void }) {
  ...
  return (
    <article
      ...
      className="panel group relative p-4 flex flex-col gap-3 cursor-pointer transition-all hover:border-orange/50 hover:shadow-[0_0_28px_rgba(255,106,26,.10)] focus-ring"
    >
      ...
      <div className="flex flex-wrap gap-1">
        {project.data_types.map((d) => <Chip key={d} tone="orange">{d}</Chip>)}
      ...
      <div className="flex items-center gap-1.5" aria-label="Stage status">
        {STAGES.map((st) => {
          ...
          return (
            <span key={st.n} title={`${st.title}: ${status}`} className={clsx('h-1.5 flex-1 rounded-full', status === 'done' && 'bg-orange shadow-[0_0_6px_rgba(255,106,26,.6)]', status === 'running' && 'bg-ok pulse-dot', status === 'paused' && 'bg-amber shadow-[0_0_6px_rgba(255,160,64,.6)] ring-1 ring-amber/50', status === 'failed' && 'bg-red', status === 'todo' && 'bg-line2')} />
          )
        })}
        <span className="font-mono text-[10px] text-muted ml-1 tabular-nums">{done}/8</span>
      </div>
      <div className="grid grid-cols-3 gap-2 font-mono text-[11px]">
        <div><div className="label !text-[9px]">rows</div><div className="text-orange text-[15px] tabular-nums">{s ? num(s.rows) : '—'}<span className="text-dim text-[10px]"> / {s ? num(s.target_rows) : '—'}</span></div></div>
      ...
      <div className="flex flex-col gap-1">
        <div className="h-1 rounded-full bg-bg/70 border border-line overflow-hidden"><div className="h-full bg-orange" style={{ width: `${rowsPct}%` }} /></div>
      ...
      <div className="flex items-center justify-between font-mono text-[10px] text-dim">
        ...
        <span className="flex items-center gap-1 text-orange opacity-0 group-hover:opacity-100 transition-opacity">open <Icon name="arrowRight" size={11} /></span>
      </div>
    </article>
  )
}
