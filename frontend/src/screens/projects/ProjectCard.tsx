...
export function ProjectCard({ project, onDelete }: { project: Project; onDelete: () => void }) {
  ...
  const tone = spendPct >= 90 ? 'bg-red' : spendPct >= 70 ? 'bg-amber' : 'bg-acid'
  return (
    <article
      role="link" tabIndex={0} onClick={() => nav(`/p/${project.id}/${next}`)} onKeyDown={(e) => { if (e.key === 'Enter') nav(`/p/${project.id}/${next}`) }}
      className="panel group relative p-4 flex flex-col gap-3 cursor-pointer transition-all hover:border-cyan/50 hover:shadow-[0_0_28px_rgba(0,240,255,.10)] focus-ring"
    >
      ...
      <div className="flex flex-wrap gap-1">
        {project.data_types.map((d) => <Chip key={d} tone="cyan">{d}</Chip>)}
      ...
      <div className="grid grid-cols-3 gap-2 font-mono text-[11px]">
        <div><div className="label !text-[9px]">rows</div><div className="text-cyan text-[15px] tabular-nums">{s ? num(s.rows) : '—'}<span className="text-dim text-[10px]"> / {s ? num(s.target_rows) : '—'}</span></div></div>
        <div><div className="label !text-[9px]">accepted</div><div className="text-acid text-[15px] tabular-nums">{s ? num(s.accepted) : '—'}</div></div>
      ...
      <div className="flex flex-col gap-1">
        <div className="h-1 rounded-full bg-bg/70 border border-line overflow-hidden"><div className="h-full bg-cyan" style={{ width: `${rowsPct}%` }} /></div>
      ...
      <div className="flex items-center justify-between font-mono text-[10px] text-dim">
        ...
        <span className="flex items-center gap-1 text-cyan opacity-0 group-hover:opacity-100 transition-opacity">open <Icon name="arrowRight" size={11} /></span>
      </div>
    </article>
  )
}
