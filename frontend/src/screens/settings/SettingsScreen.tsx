...
const STAGE_SLOTS: { key: keyof SettingsData['default_models']; label: string }[] = [
  { key: 'taxonomy', label: 'Stage 01 · Taxonomy' }, { key: 'prompts', label: 'Stage 02 · Prompts' }, { key: 'responses', label: 'Stage 03 · Teacher' },
  { key: 'judge', label: 'Stage 05 · Judge' }, { key: 'simulated_user', label: 'Simulated user' }, { key: 'weaker', label: 'Stage 04 · Weaker model' },
]
    ...
    <>
      ...
      {draft && (
        <div className="grid grid-cols-[400px_minmax(0,1fr)] gap-5 items-start">
          ...
          <Panel title="Default model per stage" actions={<Button size="sm" variant="outline" icon="refresh" loading={refresh.isPending} onClick={() => refresh.mutate()}>Refresh catalogue</Button>}>
