  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {FORMATS.map((f) => {
        ...
        return (
          <button key={f.key} type="button" onClick={() => toggle(f.key)} className={clsx('text-left rounded-card border p-3 transition-colors focus-ring flex flex-col gap-1', on ? 'border-cyan/60 bg-cyan/10 shadow-[0_0_18px_rgba(0,240,255,.08)]' : 'border-line hover:border-line2', !fits && 'opacity-60')}>
            <div className="flex items-center justify-between"><span className={clsx('font-display font-bold uppercase text-[12.5px] tracking-[.05em]', on && 'text-cyan')}>{f.label}</span><span className={clsx('w-3.5 h-3.5 rounded-[3px] border flex items-center justify-center', on ? 'bg-cyan border-cyan text-bg' : 'border-line2')}>{on && <Icon name="check" size={10} strokeWidth={3} />}</span></div>
...
export function BundleTree({ slug, formats }: { slug: string; formats: ExportFormat[] }) {
  ...
  return (
    <pre className="font-mono text-[11.5px] leading-[1.6] text-text/85">
      ...
      {formats.map((f, i) => (
        <span key={f}>{'  '}<span className="text-cyan">{f}</span>/{'\n'}{'    '}train.jsonl{'\n'}{'    '}eval.jsonl{i < formats.length - 1 ? '\n' : '\n'}</span>
      ))}
      {'  '}<span className="text-magenta">dataset_card.md</span>{'\n'}
      {'  '}<span className="text-magenta">generation_config.yaml</span>{'\n'}
      {'  '}<span className="text-magenta">manifest.json</span>
    </pre>
  )
}
