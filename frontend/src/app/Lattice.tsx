...
interface Node { x: number; y: number; r: number; c: 'cyan' | 'magenta'; ring: boolean; seed: number }
...
export function Lattice({ seed = 4242, className }: { seed?: number; className?: string }) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      nodes = Array.from({ length: n }, () => ({ x: rnd() * W, y: rnd() * H, r: 1.2 + rnd() * 1.8, c: rnd() < 0.72 ? 'cyan' : 'magenta', ring: false, seed: rnd() * Math.PI * 2 }))
      const ringCount = 8
    ...
    const draw = (now: number) => {
      ...
      for (const b of buses) {
        ...
        ctx.strokeStyle = 'rgba(0,240,255,.14)'
      ...
      for (const c of cols) {
        ...
        c.items.forEach((s, k) => {
          const y = c.y + k * 22
          if (y < -20 || y > H) return
          ctx.fillStyle = k % 5 === 0 ? `rgba(255,43,214,${c.alpha})` : `rgba(127,247,255,${c.alpha})`
      ...
      for (const nd of nodes) {
        ...
        const col = nd.c === 'cyan' ? '0,240,255' : '255,43,214'
